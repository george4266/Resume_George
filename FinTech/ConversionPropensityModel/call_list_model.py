...
test_set.drop(columns=['OwnRent', 'StRank', 'SC_ratio', 'credit_binned'], inplace=True)
#%%
...
num_called = [2902, 2769, 2435, 2268, 1868, 1801, 1801, 1768, 1701, 1468]
num_influenced = []
...
gb_clf = GradientBoostingClassifier(n_estimators=50, learning_rate=0.05,max_features=4, max_depth=5, random_state=0)
gb_clf.fit(X_train, y_train)
predictions = gb_clf.predict_proba(X_test)

xgb_clf = XGBClassifier(n_estimators=50, learning_rate=0.05,max_features=4, max_depth=5, random_state=0)
xgb_clf.fit(X_train, y_train)
predictions = xgb_clf.predict_proba(X_test)
...
import matplotlib.pyplot as plt
plt.figure()
plt.plot(fpr, tpr, label='Random Forest (Sensitivity = %0.3f)' % logit__roc_auc1)
plt.plot([0,1], [0,1], 'r--')
plt.xlim([0,1.0])
plt.ylim([0,1.05])
plt.xlabel('False Positive Rate')
plt.ylabel('True Positive Rate')
plt.title('Propensity to Renew in 1 Month')
plt.show()
