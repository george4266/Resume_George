...
sum_Portfolio = Portfolio.groupby(['AdjustedDate'])['CreditScore'].mean()
...
sum_Portfolio.tail(20)

fig, ax = plt.subplots(figsize=(15,7))
plt.plot( 'AdjustedDate', 'CreditScore', data=sum_Portfolio, marker='', color='skyblue', linewidth=2)
plt.xlabel('Date')
plt.grid()
plt.title('Average Credit Score of Borrower')
plt.savefig('SimpleCreditScore.png')


#Credit Score by Loan Type

sum_Portfolio2 = Portfolio.groupby(['AdjustedDate','ProductType'])['CreditScore'].mean()
...
fig, ax = plt.subplots(figsize=(15,7))
sum_Portfolio2.groupby(['AdjustedDate','ProductType']).mean()['CreditScore'].unstack().plot(ax=ax)
plt.title('Average Credit Score by Loan Type')
plt.grid()
plt.legend()
plt.xlabel('Date')
plt.savefig('CreditScoreByProductType.png')
