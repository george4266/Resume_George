...
amt = orig_2018[['Unique_ContractID', 'AmountFinanced', 'ProductType', 'State', 'Tier_MultipleModels']]


# %% Overall dist

plt.figure(figsize=(16,9))
plt.title('AmountFinanced Distribution After 2018')
sns.violinplot(amt['AmountFinanced'])
...
axs = axs.flatten()
num=0
for state in amt.State.unique():
    ax = sns.violinplot(amt.loc[(amt.State == state)]['AmountFinanced'], ax=axs[num])
    ax.set_title('State - {}'.format(state))
    ax.set_xlabel('AmountFinanced')
...
axs = axs.flatten()
num=0
for prod in amt.ProductType.unique():
    ax = sns.violinplot(amt.loc[(amt.ProductType == prod)]['AmountFinanced'], ax=axs[num])
...
axs = axs.flatten()
num=0
for tier in amt.Tier_MultipleModels.unique():
    ax = sns.violinplot(amt.loc[(amt.Tier_MultipleModels == tier)]['AmountFinanced'], ax=axs[num])
...
amt_fin_state = amt.groupby(['State', 'AmountFinanced'])['Unique_ContractID'].count().to_frame().reset_index()

amt_fin_tier = amt.groupby(['Tier_MultipleModels', 'AmountFinanced'])['Unique_ContractID'].count().to_frame().reset_index()

amt_fin_prod = amt.groupby(['ProductType', 'AmountFinanced'])['Unique_ContractID'].count().to_frame().reset_index()
